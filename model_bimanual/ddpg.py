...
class SharedActor(tf.keras.Model):
    def __init__(self, a_dim, a_bound):
        ...
        self.l1 = layers.Dense(300, activation='relu')
        self.l2 = layers.Dense(200, activation='relu')
        self.out = layers.Dense(a_dim, activation='tanh')
...
class DualArmActor(tf.keras.Model):
    def __init__(self, a_dim, a_bound):
        super().__init__()
        self.a_dim = a_dim
        self.a_bound = a_bound
        self.shared = SharedActor(a_dim // 2, a_bound)
...
class Critic(tf.keras.Model):
    def __init__(self, s_dim, a_dim):
        super().__init__()
        self.state_dense = layers.Dense(150, activation='relu')
        self.action_dense = layers.Dense(150, activation='relu')
        self.concat_dense = layers.Dense(300, activation='relu')
        self.l2 = layers.Dense(200, activation='relu')
    ...
    def call(self, s, a):
        ...
        x = tf.concat([state_h, action_h], axis=1)
...
class ddpg:
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        self.actor = DualArmActor(self.a_dim, self.a_bound)
        self.actor_target = DualArmActor(self.a_dim, self.a_bound)
    ...
    def save(self):
        try:
            ...
            self.actor.save("models/actor_model", save_format="tf")
            self.critic.save("models/critic_model", save_format="tf")
    ...
    def restore(self):
        try:
            self.actor = tf.keras.models.load_model("models/actor_model")
            self.critic = tf.keras.models.load_model("models/critic_model")
        except:
            try:
                self.actor.load_weights("models/actor_weights.ckpt")
                self.critic.load_weights("models/critic_weights.ckpt")
            except Exception as e:
    ...
    def load_single_arm_weights(self, path="models/actor_model"):
        try:
            single_actor = SharedActor(self.a_dim // 2, self.a_bound)
            single_actor(tf.zeros((1, self.s_dim // 2))) 

            single_actor.load_weights(path + "/variables/variables")  

            self.actor.shared.set_weights(single_actor.get_weights())
            self.actor_target.shared.set_weights(single_actor.get_weights())

            print("Single-arm weights loaded into dual-arm actor.")
        except Exception as e:
            print(f"Failed to load single-arm weights: {e}")
